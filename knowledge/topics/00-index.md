# Topic index

Rychlá mapa znalostních okruhů. Pro učení ber nejdřív A0/A1, protože se nejčastěji opakují v minulých termínech.

## A0: nejvyšší ROI

| Téma | Proč první |
|---|---|
| [[knowledge/topics/mpi-reduce-bcast]] | Nejčastější programovací úloha, typicky dobře šablonovatelná. |
| [[knowledge/topics/pram-tipovacka]] | Často první příklad; rychlé body za čas, cenu a model. |
| [[knowledge/topics/broadcast-fifo-kauzalita]] | Opakuje se jako teorie, pseudokód i diagram. |

## A1: velké opakující se bloky

| Téma | Proč |
|---|---|
| [[knowledge/topics/razeni-prefix]] | Často simulace konkrétních kroků. |
| [[knowledge/topics/euler-tour-suffix-sums]] | Stabilní pattern pro `level`, `preorder`, potomky. |
| [[knowledge/topics/synchronizace-monitory-semafory]] | Krátká teorie + klasické pseudokódy. |
| [[knowledge/topics/architektury]] | Široké téma, ale odpovědi jsou dobře šablonovatelné. |

## B/C: doplnění

- [[knowledge/topics/distribuovane-algoritmy|Distribuované algoritmy]]
- [[knowledge/topics/pi-kalkul|Pi-kalkul]]
- [[knowledge/topics/cla|Carry-look-ahead adder]]
- [[knowledge/topics/occam|OCCAM]]
- [[knowledge/topics/mutual-exclusion|Mutual exclusion]]
- [[knowledge/topics/parallel-splitting-select|Parallel splitting / SELECT]]
- [[knowledge/topics/linda-ada|Linda / ADA]]

## Jak číst topic stránku

- **Zkouškový pattern**: podle čeho poznat typ úlohy.
- **Minimální odpověď**: co stačí napsat, když není čas.
- **Postup**: kroky řešení nebo pseudokód.
- **Mini-drill**: krátké otázky na samokontrolu.
- **Kde se to objevuje**: příklady minulých termínů.
