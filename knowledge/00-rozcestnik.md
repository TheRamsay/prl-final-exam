# PRL knowledge base

Obsidian-friendly báze pro přípravu na zkoušku z PRL. Materiály vycházejí ze složky `raw`: sylabus, četnosti témat, studentský dokument a sesbíraná zadání termínů.

## Nejrychlejší start

1. [[knowledge/01-roi-plan|ROI plán učení]]
2. [[knowledge/02-cetnosti-temat|Četnosti témat]]
3. [[knowledge/03-sylabus|Sylabus a mapa předmětu]]
4. [[knowledge/04-checklist-nejcastejsi-temata|Checklist nejčastějších témat]]
5. [[knowledge/exams/00-index|Minulé termíny]]
6. [[knowledge/topics/00-index|Topic index]]
7. [[knowledge/visuals/00-index|Vizualizace]]
8. [[knowledge/practice/00-index|Cvičné testy]]
9. [[knowledge/05-vyhledavani|Vyhledávání]]
10. [[knowledge/99-llm-index|LLM index pro vyhledávání]]

## Top témata

- [[knowledge/topics/mpi-reduce-bcast|MPI: Reduce a Bcast]]
- [[knowledge/topics/pram-tipovacka|PRAM tipovačka]]
- [[knowledge/topics/broadcast-fifo-kauzalita|Broadcast, FIFO, kauzalita, ABCAST]]
- [[knowledge/topics/architektury|Architektury: VLIW, pipeline, dataflow, SIMD/MIMD]]
- [[knowledge/topics/razeni-prefix|Řazení, prescan, prefix/suffix]]
- [[knowledge/topics/synchronizace-monitory-semafory|Monitory, semafory, čtenáři/písaři]]
- [[knowledge/topics/euler-tour-suffix-sums|Euler tour a suffix sums]]
- [[knowledge/topics/distribuovane-algoritmy|Distribuované algoritmy]]
- [[knowledge/topics/pi-kalkul|Pi-kalkul]]
- [[knowledge/topics/cla|Carry-look-ahead adder]]
- [[knowledge/topics/occam|OCCAM]]

## Jak to používat

- Každé téma má sekce: co umět, typické zadání, šablona odpovědi, chyby.
- Každý termín má jednotný formát a odkazy na témata.
- Obrázky z `raw/*.webp` jsou zkopírované v `assets/`.
- `raw/student_doc.md` je zatím vedený jako zdroj a index, protože obsahuje velké vložené base64 obrázky.
