...
| Auditovatelný extract | [[knowledge/sources/student-doc/2021-2022-extract]] |
...
- Extract roku: [[knowledge/sources/student-doc/2021-2022-extract]]
...
3. [[knowledge/topics/euler-tour-suffix-sums|Etour + suffix sum]] - určit úroveň uzlu ve stromu.
4. [[knowledge/topics/razeni-prefix|Pipeline Merge Sort]].
   - Zdroj připomíná `p(n)=log n + 1`, procesor `Pi` spojuje dvě seřazené posloupnosti délky `2^(i-2)`, čas `O(n)`, cena `O(n log n)`.
5. [[knowledge/topics/architektury|Zřetězené procesory / MISD]].
6. [[knowledge/topics/broadcast-fifo-kauzalita|Asynchronní komunikace na synchronní]] - rozhodnout, zda je převod možný.
7. [[knowledge/topics/occam|OCCAM]] - buffer, overflow nebo hodnota `0`, výstup na `high/low` podle thresholdu.
8. [[knowledge/topics/mpi-reduce-bcast|MPI]] - zřejmě `sum % max == 0` nebo `sum % min == 0`, stačí 2x `Reduce`.

### Term 1 - řádný termín - skupina B

1. [[knowledge/topics/pram-tipovacka|PRAM tipsport]].
2. [[knowledge/topics/architektury|Vektorové procesory SIMD]] - výhody, nevýhody, obrázek UMA/NUMA.
3. [[knowledge/topics/parallel-splitting-select|Paralelní SELECT k-tého prvku]].
   - Zdroj uvádí EREW PRAM, sdílené pole, `t(n)=O(n/N)` pro omezený počet procesorů, cena `O(n)`.
4. [[knowledge/topics/mutual-exclusion|Petersonův algoritmus]] - princip a pseudokód.
5. Synchronizace.
6. [[knowledge/topics/razeni-prefix|Enumeration Sort]] - napsat registry `X, Y, C, Z` po skončení algoritmu.
7. [[knowledge/topics/occam|OCCAM]] - pole 10 kanálů, vstupy `adr`, `in`; hodnoty z `in` posílat na aktuální kanál, `adr` posouvá index modulo 10.
8. [[knowledge/topics/mpi-reduce-bcast|MPI]] - pokud `max % min == 0`, vypsat `Ano`, jinak `Ne`.
...
5. [[knowledge/topics/broadcast-fifo-kauzalita|Asynchronní signály -> synchronní provedení]].
   - Zdroj: hledat korunu; pokud koruna existuje, systém nelze provést synchronně.
6. [[knowledge/topics/razeni-prefix|Prescan]] - zapsat jako pole, vědět, které pozice se přepisují.
7. [[knowledge/topics/occam|OCCAM]] - kanály `1-10`, vstupy `adr` a `in`; `adr` nastaví aktivní index, `in` posílá na aktivní kanál.
8. [[knowledge/topics/mpi-reduce-bcast|MPI]] - počet prvků rovných maximu a počet prvků rovných minimu v logaritmickém čase.

### Term 2 - 1. opravný - skupina A

Samostatný soubor: [[knowledge/exams/2021-2022/term-2-prvni-opravny-a]]

1. [[knowledge/topics/pram-tipovacka|CRCW AND]] - popsat algoritmus a uvést příklad.
2. [[knowledge/topics/architektury|MIMD / Xeon Phi]] - kde využít, popis a obrázek.
3. [[knowledge/topics/euler-tour-suffix-sums|Suffixsum pro Euler path]] - výpočet úrovně vrcholu.
...
7. [[knowledge/topics/occam|OCCAM]] - `OUT[10]`, `IN`, `ALT`; `ALT` zapíná/vypíná alternování mezi kanály.
8. [[knowledge/topics/mpi-reduce-bcast|MPI]] - sudé procesory mají sudé prvky a liché procesory liché prvky.

### Term 3 - 2. opravný termín

Samostatný soubor: [[knowledge/exams/2021-2022/term-3-druhy-opravny]]
...
- U async -> sync se opakuje motiv koruny, tj. důležité pro [[knowledge/topics/broadcast-fifo-kauzalita]].
